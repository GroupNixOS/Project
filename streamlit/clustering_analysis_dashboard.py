...
#PCA integrated data
pca_query = "SELECT * FROM pca" #Can replace * with specifics. 
...
#PVE integrated data
pve_query = "SELECT * FROM pve" #Can replace * with specifics. 
...
st.subheader("Variance captured per component")

#st.write(pve_df) #Tutors have indicated that the graph is sufficient. 
...
with st.expander("Superpopulations Analysis"):
    ...
    xsvar = st.selectbox('Select x-axis:', merged_df.columns[0:21], key='xs_selectbox') #sliced columns to hide "Population" in selectbox from user. 
    ysvar = st.selectbox('Select y-axis:', merged_df.columns[0:21], key='ys_selectbox')
    zsvar = st.selectbox('Select z-axis:', merged_df.columns[0:21], key='zs_selectbox')
